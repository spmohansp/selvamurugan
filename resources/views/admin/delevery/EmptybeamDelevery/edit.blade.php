...
@section('content')
    <div class="row page-titles">
        <div class="col-md-12 align-self-center">
            <h4 class="theme-cl">Update Empty Beam Delevery</h4>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12">
            <form data-toggle="validator" class="padd-20" method="post" action="{{ route('admin.EmptyBeamDeleveryUpdate',$EmptyBeamDelevery->id) }}" >
                <div class="card">
                    {{ csrf_field() }}
                    <div class="row page-titles">
                ...
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <select name="unit_id" class="form-control" required>
                            ...
                            <option value="{{ $Unit->id }}"{{ ($Unit->id == $EmptyBeamDelevery->unit_id) ?'selected':'' }} >{{ $Unit->name }}</option>
                            @endforeach
                         </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="customer_id" class="form-control SearchableDropDownSelect" id="CustomerIdsChanges" required>
                                    ...
                                    @foreach(auth()->user()->getAllCustomers() as $Customer)
                                        <option value="{{ $Customer->id }}" {{ ($Customer->id == $EmptyBeamDelevery->customer_id)? 'selected':'' }}>{{ $Customer->name }} | {{ $Customer->mobile }}</option>
                                    @endforeach
                                </select>
                                <div class="help-block with-errors"></div>
                            </div>
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="date"  value="{{ $EmptyBeamDelevery->date }}"  required="" >
                                <div class="help-block with-errors"></div>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="number" name="beam_total" class="form-control" value="{{ $EmptyBeamDelevery->beam_total }}" required="">
                                <div class="help-block with-errors"></div>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" name="beam_inch"  value="{{ $EmptyBeamDelevery->beam_inch }}" required="" >
                                <div class="help-block with-errors"></div>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <textarea name="note" class="form-control">{{$EmptyBeamDelevery->note}}</textarea>
                            </div>
                        </div>
                    </div>
...
@section('scriptOnload')
    <script>
        $(document).ready(function () {
            ...
            @endif()
            setTimeout(function(){
                $("#sub_customer_id option[value='<?php echo $EmptyBeamDelevery->sub_customer_id ?>']").attr("selected",true);
            }, 500);
        });
    </script>
@endsection
